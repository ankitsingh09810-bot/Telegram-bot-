...
# /start Command with Japanese UI
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ask_login":
        ...
        await query.message.edit_text(
            ...
            "Aapko login karne ke liye niche diye gaye button par click karein ya fir password type karein:\n"
            "`/login ANKIT MERA BAAP HA`",
        ...
        # Direct bypass for owner using the button
...
async def show_dashboard(query):
    keyboard = [
        [InlineKeyboardButton("📁 Upload .py Script (Drag & Drop)", callback_data="info_upload")],
    ...
    await query.message.edit_text(
        "🌟 **[ DASHBOARD UNLOCKED ]** 🌟\n"
        "Aapka system fully operational hai. Ab aap apni `.py` file direct yahan bhej sakte hain! 🚀",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown"
    )

# /login command (agar koi type kare)
...
print("Japanese UI Cyberpunk Hosting Bot Started...")
app.run_polling()
